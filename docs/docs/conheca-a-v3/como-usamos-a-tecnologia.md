...
Atualmente, nosso foco principal está na utilização de dashcams (câmeras de painel) de alta qualidade e baixo custo instaladas em veículos leves. Estas câmeras:
...
-  **Podem se integrar com o veículo**: Podem coletar dados adicionais via protocolo CAN (Controller Area Network), permitindo acesso a informações como velocidade, RPM, aceleração, frenagem, status de cintos de segurança e outros dados do veículo em tempo real
...
Usamos, desenvolvemos e implementamos modelos de inteligência artificial permitem a criação de soluções que:
...
Como especialistas em visão computacional para mobilidade, pesquisamos e desenvolvemos soluções que transformam a maneira como veículos e infraestruturas interagem. Nossa solução está em constante evolução, e estamos projetando nossa solução para suportar diversos cenários. Alguns exemplos práticos incluem:
