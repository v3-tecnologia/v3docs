...
- **Criações do Recursos**: Nossas APIs quase sempre trabalham com um sistema de enfileiramento para a criação e preparação dos dados. Por isso, ao realizar uma operação de `POST` é comum que você não receba imediamente um `Status Code: 201` que é o mais tradicional para as APIs convencionais, mas sim um `Status Code: 202` que significa que o recursos será tratado em breve.
...
- **Especificação api (Swagger)**: Documentação estruturada no padrão da indústria
...
Para detalhes completos sobre as APIs disponíveis, consulte a [Seção API](/docs/category/order-api) desta documentação, onde você encontrará informações detalhadas sobre as APIs disponíveis.

## Integração com Sistemas de Terceiros

Nossas APIs permitem integração perfeita com clientes externos, através de [Webhooks](/docs/docs/integracao-e-operacao/api/notificacoes). Os detalhes de Integração estão disponível na [Seção Notifications](/docs/category/notification-webhook)
