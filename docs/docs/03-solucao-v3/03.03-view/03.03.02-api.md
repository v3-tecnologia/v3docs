...
- **🎟️ Tokens JWT**: JSON Web Tokens para transmissão segura de claims entre sistemas
- **⚖️ Escopos Granulares**: Controle fino sobre permissões para cada endpoint
...
- **⏱️ Rate Limiting**: Limites configuráveis de requisições por período para evitar sobrecarga
...
- **🖥️ Swagger UI**: Interface interativa para exploração e teste dos endpoints
...
Para detalhes completos sobre as APIs disponíveis, consulte a [Seção API](/docs/category/orders-api) desta documentação, onde você encontrará informações detalhadas sobre as APIs disponíveis.

## 🔄 Integração com Sistemas de Terceiros

Nossas APIs permitem integração perfeita com clientes externos, através de [Webhooks](/docs/docs/13-webhooks.md). Os detalhes de Integração estão disponível na [Seção Notifications](/docs/category/notifications-webhook)
